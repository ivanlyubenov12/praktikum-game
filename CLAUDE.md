...
- **`TEMPLATES`/`COLORS` are defaults, not what the game actually plays.** The in-app content editor
  (`docs/EDITOR.md`) layers player edits from `localStorage` on top of them into
  `ACTIVE_TEMPLATES`/`ACTIVE_COLORS`/`ACTIVE_SCORING`. **Runtime code must read the `ACTIVE_*` globals, never
  the defaults directly** — `generateSet`, `loadEq`, `renderCounter`, `check`, `hint`, `updateScore`, `finish`
  all do this already. New runtime code should too.
